...
const video_metadata_service_1 = require("../video-metadata/video-metadata.service");
...
let AdminService = AdminService_1 = class AdminService {
    ...
    constructor(prisma, cmsService, videoMetadataService) {
        this.prisma = prisma;
    ...
    async getDashboardStats() {
        try {
            const [totalUsers, totalCourses, totalRevenue, pendingDeviceRequests, recentUsers, recentCourses] = await Promise.all([
            ...
            return {
                users: {
                    total: totalUsers,
                    students: await this.prisma.user.count({ where: { role: client_1.UserRole.STUDENT } }),
                    instructors: await this.prisma.user.count({ where: { role: client_1.UserRole.INSTRUCTOR } }),
                    admins: await this.prisma.user.count({ where: { role: client_1.UserRole.ADMIN } }),
                },
                courses: {
                    total: totalCourses,
    ...
    async denyDeviceRequest(requestId) {
        try {
    ...
    async getUserDevicesByUserId(userId) {
        try {
    ...
    async createCourse(createCourseDto) {
        try {
            ...
            if (createCourseDto.taxIncluded !== undefined && createCourseDto.price) {
                const priceCalculation = await this.calculatePriceWithTax(createCourseDto.price, createCourseDto.taxIncluded === 'included');
    ...
    async calculateCourseDuration(courseId) {
        try {
            ...
            return {
                ...
                durationInMinutes: Math.round(totalDuration / 60 * 100) / 100,
                durationInHours: totalDuration >= 3600 ? Math.floor(totalDuration / 3600) : 0,
    ...
    async getTaxRate() {
        try {
            ...
            return Number(settings.tax_rate) || 20;
        }
        catch (error) {
    ...
    async calculatePriceWithTax(price, taxIncluded = true) {
    ...
    async getVideoDuration(videoUrl) {
        try {
            ...
            const returnData = {
                durationInSeconds,
                durationInMinutes: Math.round(durationInMinutes * 100) / 100,
                formattedDuration
            };
    ...
    async approveDeviceRequest(requestId, body) {
        try {
